...
void MainWindow::connectevent()
{
    ...
    {
        QProcess process;
//        process.startDetached("taskkill /im UartAssist.exe /f");
        process.startDetached(QApplication::applicationDirPath()+"/uart/UartAssist.exe");
        process.close();
    });
...
//        if(serial->waitForReadyRead(3000))
        pRetVal = serial->readData(data,13);
        if(pRetVal > 0)
        {
            QString strTmp;
            for(int i = 0; i<13; i++)
...
//                    map_Factors["烟尘排放量"]->m_value = QString::number( map_Factors["烟尘湿值"]->m_value.toDouble() * map_Factors["标况流量"]->m_value.toDouble() / 1000000,'f',2);
                    ...
                    if(map_Factors.contains("非甲烷总烃"))
                    {
                        ...
                        if(map_Factors.contains("非甲烷总烃干值")&&map_Factors.contains("烟气湿度"))
                        {
                            ...
                            if(map_Factors.contains("折算非甲烷总烃"))
                            {
...
//        QThread::sleep(3);
        ...
        int pRetVal;
//        if(serial->waitForReadyRead(3000))
        pRetVal = serial->readData(data,37);
        if(pRetVal > 0)
        {
            QString strTmp;
            for(int i = 0; i<37; i++)
...
bool MainWindow::eventFilter(QObject *obj, QEvent *event)
{
    ...
    if(obj == ui->pushButton_5)
    {
        if(event->type() == QEvent::HoverEnter)
        {
            QToolTip::showText(QCursor::pos(), "串口调试");
            return true;
        }
    }
